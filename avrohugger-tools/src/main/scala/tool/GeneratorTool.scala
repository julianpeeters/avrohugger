...
class GeneratorTool(sourceFormat: SourceFormat,
  ...
  val generator = Generator(sourceFormat, avroScalaCustomTypes, avroScalaCustomNamespace)
  ...
  def run(in: InputStream, out: PrintStream, err: PrintStream, args: List[String]): Int = {
    if (args.size() < 3) {
      ...
      val inputs: List[File] = new ArrayList[File]()
  ...
  val SCHEMA_FILTER: FileExtensionFilter = FileExtensionFilter("avsc")
  val PROTOCOL_FILTER: FileExtensionFilter = FileExtensionFilter("avpr")
  val DATAFILE_FILTER: FileExtensionFilter = FileExtensionFilter("avro")
