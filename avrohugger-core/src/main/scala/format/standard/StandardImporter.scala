  ...
  private[this] def getShapelessImports(
    ...
    def coproductImportsForUnionType(
      ...
      def defaultValueTest(field: Schema.Field, unionTypes: List[Schema], maxNonNullTypes: Int) =
        (unionTypes.length > maxNonNullTypes     && !unionTypes.exists(unionContainsNull) && field.defaultVal != null) ||
        (unionTypes.length > maxNonNullTypes + 1 &&  unionTypes.exists(unionContainsNull) && field.defaultVal != null)
