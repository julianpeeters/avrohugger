...
package example.idl.string.model

import org.apache.avro.Schema

import org.oedura.scavro.{AvroMetadata, AvroReader, AvroSerializeable}

import example.idl.{DefaultTest => JDefaultTest, Embedded => JEmbedded}
...
final case class DefaultTest(suit: String = "SPADES", number: Int = 0, str: String = "str", optionString: Option[String] = None, optionStringValue: Option[String] = Some("default"), embedded: Embedded = new Embedded(1), defaultArray: Array[Int] = Array(1, 3, 4, 5), optionalEnum: Option[String] = None, defaultMap: Map[String, String] = Map("Hello" -> "world", "Merry" -> "Christmas"), byt: Array[Byte] = Array[Byte](-61, -65)) extends AvroSerializeable with Defaults {
  type J = JDefaultTest
  override def toAvro: JDefaultTest = {
    new JDefaultTest(suit, number, str, optionString match {
      case Some(x) => x
      case None => null
...
final object DefaultTest {
  ...
  implicit val metadata: AvroMetadata[DefaultTest, JDefaultTest] = new AvroMetadata[DefaultTest, JDefaultTest] {
    ...
    override val fromAvro: (JDefaultTest) => DefaultTest = {
      (j: JDefaultTest) => DefaultTest(j.getSuit.toString, j.getNumber.toInt, j.getStr.toString, j.getOptionString match {
        case null => None
        ...
        case null => None
        case _ => Some(j.getOptionalEnum.toString)
