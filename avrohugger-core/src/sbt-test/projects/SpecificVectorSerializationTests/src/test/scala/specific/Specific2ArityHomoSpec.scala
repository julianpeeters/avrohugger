...
class Specific2ArityHomoSpec extends Specification {
  ...
  "A case class with an `Vector[String]` field in the second position" should {
    "serialize and deserialize correctly" in {
      val record1 = AvroTypeProviderTest24(Vector("mekka.lekka.hi"), Vector("mekka.hiney.ho"))
      val record2 = AvroTypeProviderTest24(Vector("time"), Vector("travel"))
      val records = List(record1, record2)
      SpecificTestUtil.verifyWriteAndRead(records)
    }
  }

  "A case class with an `Vector[Int]` field in the second position" should {
    "serialize and deserialize correctly" in {
      val record1 = AvroTypeProviderTest25(Vector(1, 2), Vector(3,4))
      val record2 = AvroTypeProviderTest25(Vector(5, 6), Vector(7,8))
      val records = List(record1, record2)
      SpecificTestUtil.verifyWriteAndRead(records)
    }
  }
  ...
  "A case class with two Map[String, Option[Vector[Int]]] fields" should {
    "serialize and deserialize correctly" in {
      val record1 = AvroTypeProviderTestMap06(Map("Olala"->Some(Vector(1,4))), Map("Rumpole"->None))
      val record2 = AvroTypeProviderTestMap06(Map("Cran"->Some(Vector(3,5))), Map("Doc"->None))
      val records = List(record1, record2)
      SpecificTestUtil.verifyWriteAndRead(records)
    }
  }
